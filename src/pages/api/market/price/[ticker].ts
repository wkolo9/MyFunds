...
export const GET: APIRoute = async (context) => {
  try {
    ...
    if (!user) {
      return createErrorResponseObject(ErrorCode.INVALID_TOKEN, "Unauthorized", 401);
    }
    ...
    if (!validationResult.success) {
      return createErrorResponseObject(ErrorCode.VALIDATION_ERROR, "Invalid ticker format", 400, "ticker");
    }

    const validTicker = validationResult.data.ticker;

    // 3. Service Call
    const priceData = await marketService.getPrice(validTicker);
