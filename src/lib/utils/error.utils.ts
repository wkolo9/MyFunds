...
export function handleServiceError(error: unknown): Response {
  ...
  if (error instanceof ConflictError) {
    return createErrorResponseObject(ErrorCode.CONFLICT, error.message, 409);
  }

  if (error instanceof Error) {
    // Handle specific service errors
    if (error.message === "Profile not found") {
      return createErrorResponseObject(ErrorCode.PROFILE_NOT_FOUND, "Profile not found", 404);
    }

    if (error.message === "Sector not found") {
      return createErrorResponseObject(ErrorCode.SECTOR_NOT_FOUND, "Sector not found", 404);
    }

    if (error.message === "Asset not found") {
      return createErrorResponseObject(ErrorCode.ASSET_NOT_FOUND, "Asset not found", 404);
    }

    if (error.message === "Watchlist item not found") {
      return createErrorResponseObject(ErrorCode.WATCHLIST_ITEM_NOT_FOUND, "Watchlist item not found", 404);
    }

    if (error.message.startsWith("Database error:")) {
      return createErrorResponseObject(ErrorCode.DATABASE_ERROR, "Internal server error", 500);
    }
  ...
  return createErrorResponseObject(ErrorCode.INTERNAL_ERROR, "Internal server error", 500);
}
