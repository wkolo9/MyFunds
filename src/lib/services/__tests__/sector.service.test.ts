...
describe("SectorService", () => {
  ...
  describe("createSector", () => {
    ...
    it("should throw error if sector limit reached", async () => {
      ...
      await expect(sectorService.createSector(userId, { name: "Tech" })).rejects.toThrow(
        "Maximum limit of 32 sectors reached"
      );
    });
    ...
    it("should throw error if name already exists", async () => {
      ...
      (mockSupabase.from as any).mockReturnValueOnce(mockCountChain).mockReturnValueOnce(mockUniqueChain);

      await expect(sectorService.createSector(userId, { name: "Tech" })).rejects.toThrow(
        'Sector with name "Tech" already exists'
      );
    });
  });
  ...
  describe("updateSector", () => {
    ...
    it("should throw not found if sector does not exist", async () => {
      ...
      await expect(sectorService.updateSector(userId, sectorId, { name: "New" })).rejects.toThrow("Sector not found");
    });
  });
  ...
  describe("deleteSector", () => {
    it("should delete sector if exists", async () => {
      ...
      (mockSupabase.from as any).mockReturnValueOnce(mockFindChain).mockReturnValueOnce({
