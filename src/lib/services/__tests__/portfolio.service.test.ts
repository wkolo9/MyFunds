...
describe("PortfolioService", () => {
  ...
  describe("createAsset", () => {
    ...
    it("should throw conflict if ticker exists", async () => {
      ...
      // Execute & Verify
      await expect(service.createAsset(userId, { ticker: "AAPL", quantity: "5" })).rejects.toThrow(ConflictError);
    });
    ...
    it("should throw validation error if ticker invalid", async () => {
      ...
      // Execute & Verify
      await expect(service.createAsset(userId, { ticker: "INVALID", quantity: "5" })).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe("updateAsset", () => {
    it("should update asset", async () => {
      ...
      mockSupabase
        ...
        .select()
        .eq()
        .maybeSingle.mockResolvedValue({ data: { id: "1" } }); // Exists
  ...
  describe("deleteAsset", () => {
    ...
    it("should throw not found if asset does not exist", async () => {
      ...
      // Execute & Verify
      await expect(service.deleteAsset(userId, "999")).rejects.toThrow(NotFoundError);
    });
  });
});
